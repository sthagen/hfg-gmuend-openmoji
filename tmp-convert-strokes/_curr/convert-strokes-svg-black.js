    ...
    if (isSet(attrStroke) && !isSet(attrFill)) {
      ...
      if (el.tagName == 'path') {
        ...
        if (l > 1) {
          for (var i = 0; i < l - 2; i++) {
            let pp = p.clone();
            if (i < l - 2) pp.removeSegments(i + 2);
            if (i > 0) pp.removeSegments(0, i);

            console.log(pp.segments[0].toString());
            console.log(pp.segments[1].toString());
            try {
              if (pp.length > 0) {
                let res = offsetStroke(pp, offset, attrStroke, attrFill, attrCap, attrJoin);
                insertAfter(res, el);
              }
            } catch (e) {
              // statements
